...
Repository write access alone cannot broaden app authentication. After committing and pushing the intended dev change, an authorized floo org-admin agent can run `floo redeploy --app <app> --rebuild` with an admin-scoped credential to apply the current GitHub default-branch HEAD. The access transition needs no dashboard approval; any separately configured infrastructure-review policy still applies. Rebuilding dev does not apply production access changes.
